...
void iAnt_controller::shutdown() {
	...
	if((GetPosition() - target).SquareLength() < (data->ArenaX+data->ArenaY)/3.0 ||
       searchTime % (data->TicksPerSecond * 30) == 0) {
...
void iAnt_controller::SetHoldingFood() {
    ...
    if(!IsHoldingFood()) {
        ...
        size_t i = 0, j = 0;
		if(CPFA!= RETURNING){ //qilu 07/04/2015  remove the case of finding food when the robot returns to the nest
			for(i = 0; i < data->FoodList.size(); i++) {
				if((GetPosition() - data->FoodList[i]).SquareLength() < data->FoodRadiusSquared) {
					...
					//LOG<<"Return this unfound-food position to the list"<<endl;
...
void iAnt_controller::SetLocalResourceDensity() {
    ...
    //vector<CVector2> newFidelityList; //qilu 07/17
    ...
    //data->FidelityList = newFidelityList; //qilu 07/17
