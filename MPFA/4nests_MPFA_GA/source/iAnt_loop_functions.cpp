...
void iAnt_loop_functions::Init(TConfigurationNode& node) {
	...
	data.CollisionTimeList.clear(); //qilu 08/19
...
void iAnt_loop_functions::PostExperiment() {
    ...
    /* if(data.OutputData == 1) {
        ...
        dataOutput <<data.CollisionTime/16.0<<", "<< time_in_minutes << ", " << data.RandomSeed << endl;//qilu 08/19
        dataOutput.close();
		//qilu 08/19
        ...
        ofstream collisionDataOutput("CollisionData.txt", ios::app);
        if(data.CollisionTimeList.size()!=0) collisionDataOutput<<"Collision: "<< data.CollisionTimeList[0]/16.0;
        for(size_t i=1; i< data.CollisionTimeList.size(); i++) collisionDataOutput<<", "<<data.CollisionTimeList[i]/16.0;
        collisionDataOutput<<"\n";
        collisionDataOutput.close();
        //qilu 08/19
        
        //qilu 10/22
        size_t remainderTime;
        ofstream travelSearchTimeDataOutput("TravelSearchTimeData.txt", ios::app);
        travelSearchTimeDataOutput<<"Traveling time, Searching time"<<endl;
        for(size_t i = 0; i < iAnts.size(); i++) {
            remainderTime = data.SimTime-iAnts[i]->GetTravelingTime()-iAnts[i]->GetSearchingTime();
            if (iAnts[i]->GetStatus() == "DEPARTING" || iAnts[i]->GetStatus() == "RETURNING") {
                iAnts[i]->AddTravelingTime(remainderTime);
            } else if(iAnts[i]->GetStatus() == "SEARCHING"){
                iAnts[i]->AddSearchingTime(remainderTime);
            }
            travelSearchTimeDataOutput<< iAnts[i]->GetTravelingTime()<<", "<<iAnts[i]->GetSearchingTime()<<endl;
            
            //LOG<<"searching time="<<iAnts[i]->GetSearchingTime()<<", "<<"traveling time="<<iAnts[i]->GetTravelingTime()<<endl;
        }//qilu 10/22
    } */
    ...
    if(data.SimCounter == 0) {
        ...
        LOG << collectedFood << ", ";
        LOG <<data.CollisionTime/16.0<<", "<<time_in_minutes << ", " << data.RandomSeed << endl; //qilu 08/19
    } else {
        LOG << collectedFood << ", ";
        LOG << data.CollisionTime/16.0<<", "<< time_in_minutes << ", " << data.RandomSeed << endl; //qilu 08/19
    }
