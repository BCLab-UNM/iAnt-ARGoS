...
size_t iAnt_controller::GetSearchingTime(){//qilu 10/22
    return searchingTime;
}
size_t iAnt_controller::GetTravelingTime(){//qilu 10/22
...
void iAnt_controller::searching() {
    ...
    if(!IsHoldingFood()) { //qilu, every tick in this branch
        CVector2 distance = GetPosition() - target;
...
//                Real rand = RNG->Gaussian(USCV);
            ...
            /* informed search */
                ...
                Real   t           = searchTime++;// qilu 01/20/2016 time in 5 seconds for search decay
    ...
    /* Food has been found, change state to RETURNING and go to the nest */
        ...
        string key = to_string(target.GetX())+", "+to_string(target.GetY());

        if (data->redundantNests.find(key) == data->redundantNests.end())
            data->redundantNests[key] =1; // not found
    ...
    /* Are we there yet? (To the nest, that is.) */
   	if(distance.SquareLength() < data->NestRadiusSquared) {
		...
		if(poissonCDF_pLayRate > r1 && updateFidelity){//qilu 07/29
			Real timeInSeconds = (Real)(data->SimTime / (5*data->TicksPerSecond));//qilu 01/20/2016 change to each 5 seconds
...
bool iAnt_controller::SetTargetPheromone() {
    ...
    if(ClosestNest->PheromoneList.size()==0) return isPheromoneSet; //qilu 07/04 the case of no pheromone.  
	...
	for(size_t i = 0; i < ClosestNest->PheromoneList.size(); i++) {
        if(ClosestNest->PheromoneList[i].IsActive()) { //qilu 07/05
 ...
 *****/
Real iAnt_controller::GetExponentialDecay(Real w, Real time, Real lambda) {//qilu 01/08/2016
...
string iAnt_controller::GetStatus(){//qilu 10/22
    //DEPARTING, SEARCHING, RETURNING
    if (MPFA == DEPARTING) return "DEPARTING";
    else if (MPFA==SEARCHING)return "SEARCHING";
    else if (MPFA == RETURNING)return "RETURNING";
    else if (MPFA == INACTIVE) return "INACTIVE";
    else return "SHUTDOWN";
    
}

void iAnt_controller::AddTravelingTime(size_t remainderTime){//qilu 10/22
    travelingTime += remainderTime;
}
void iAnt_controller::AddSearchingTime(size_t remainderTime){//qilu 10/22
...
bool iAnt_controller::isCollisionOnWall(){
    ...
    if (maxX - GetPosition().GetX()<=0.3 || GetPosition().GetX()+maxX<=0.3 || maxY - GetPosition().GetY()<=0.3 || GetPosition().GetY()+maxY<=0.3)
        //LOG<<"Collision on wall"<<endl;
		return true;
	 return false;
	 }
/*****
