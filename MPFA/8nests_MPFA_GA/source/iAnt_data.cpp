...
iAnt_data::iAnt_data() :
    ...
    currCollisionTime(0), //qilu 10/30
    lastCollisionTime(0), //qilu 10/30
    currNumCollectedFood(0), //qilu 08/19
    lastNumCollectedFood(0), //qilu 08/19
    CollisionTime(0),//qilu 10/30
    TicksPerSecond(16),
   ...
   vector<iAnt_pheromone> new_p_list;
	for(size_t n=0; n<nests.size(); n++){
		for(size_t i = 0; i < nests[n].PheromoneList.size(); i++) {
			nests[n].PheromoneList[i].Update((Real)(SimTime /(5*TicksPerSecond)));
...
bool iAnt_data::IsCollidingWithNest(CVector2 p) {
    ...
    for(size_t i = 0; i < nests.size(); i++) {//qilu 07/05
