...
if __name__ == "__main__":
    ...
    this_run = Random_Argos("./experiments/iAnt_linux.argos")
    output_fileName="MPFA_result.txt"
    output_file=open(output_fileName, 'w')
    for arena_width in [10]:
        output_file.write("arena size: "+str(arena_width)+" * " +str(arena_width)+" ")
        print "arena size: "+str(arena_width)+" * " +str(arena_width)
        #this_run.update_arena_size(arena_width)
        #this_run.update_nest_position(arena_width)
        #this_run.update_robot_position(arena_width)
	count =0
        for _ in range(5):
            count = count+1
